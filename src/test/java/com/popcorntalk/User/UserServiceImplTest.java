...
public class UserServiceImplTest extends MockData {
    ...
    void validateAdminSuccess() {
        ...
        //given
        boolean flag = true;
        given(mockRepo.validateAdminUser(TEST_USER_ID)).willReturn(
            TEST_ADMIN_USER.getRole().equals(UserRoleEnum.ADMIN));
    ...
    void getMyInfo() {
        ...
        assertEquals("개인 유저가 자신의 일일 횟수 보기 1번 포스트 했기에 갑은 2", 2,
            userInfoResponseDto.getDailyPostsLimit());
    }
    ...
    void getUserInfo() {
        ...
        // when
        UserPublicInfoResponseDto userPublicInfoResponseDto1 = userService.getUserInfo(
            TEST_USER_ID);

        // then
        assertEquals("개인 유저가 자신의 이메일 보기 ", userPublicInfoResponseDto1.getEmail(),
            TEST_GET_USER_RESPONSE_DTO.getEmail());
    }
    ...
    void getAllUserInfo() {
        ...
        Pageable pageable = Pageable.ofSize(10)
            .withPage(0); // Assuming page size is 10 and page number is 0
        List<UserPublicInfoResponseDto> userList = new ArrayList<>();
        userList.add(TEST_GET_USER_RESPONSE_DTO);
        userList.add(TEST_GET_USER_RESPONSE_DTO);
        userList.add(TEST_GET_USER_RESPONSE_DTO);

        Page<UserPublicInfoResponseDto> userPage = new PageImpl<>(userList, pageable,
            userList.size());
        //when
        UserServiceImpl userService = new UserServiceImpl(mockRepo, passwordEncoder, pointService,
            postRecodeService);
